(ns resinmfg.advisor
  "ResinAdvisor -- the *contained intelligence node* for the plastics-
  and-synthetic-rubber PRIMARY FORMS plant-operations coordination
  actor.

  It normalizes production-batch patches (polymer-grade/weight/off-
  spec-rate), drafts a polymerization/compounding-reactor maintenance
  scheduling proposal against a piece of equipment, drafts a safety-
  concern flag, and drafts an outbound resin/synthetic-rubber primary-
  form shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real reactor/polymerization-line
  actuation or freight dispatch. Every output is censored downstream
  by `resinmfg.governor` before anything touches the SSoT -- see
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct reactor/
                                 ; polymerization-line-control effect
  ...
  contract every caller of this actor agrees to) -- `resinmfg.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [resinmfg.registry :as registry]
            [resinmfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's polymer-grade,
  weight, off-spec-rate or verification status. High confidence, low
...
(defn- schedule-maintenance
  "Draft a polymerization/compounding-reactor maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `resinmfg.governor` NEVER trusts this report -- it
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-reactor?=" (boolean (:actuate-reactor? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-reactor? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a chemical-hazard (monomer-exposure/exothermic-reaction-risk)/
  ...
  on an administrative technicality). See `resinmfg.phase`: no phase
  ever adds this op to a phase's `:auto` set; `resinmfg.governor` also
...
(def ^:private system-prompt
  (str "あなたはプラスチック・合成ゴム一次製品(primary forms)工場プラント運用"
       "コーディネーターの助言者です。"
       ...
       "重合反応器・コンパウンド設備等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `resinmfg.governor`
