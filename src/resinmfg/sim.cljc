(ns resinmfg.sim
  ...
  polymerization/compounding reactor directly (permanently blocked, no
  override), a double-schedule of the same maintenance window, a
  production-batch patch with a fabricated polymer grade, and a
  production-batch patch with an implausible off-spec-rate reading.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on reactor-001 (verified, registered, polymerization-reactor -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on reactor-001 (always escalates -- approve) ==")
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:polymer-grade :pp}}
                       coordinator))

    (println "== unrecognized op -> HARD hold ==")
    (println (exec-op actor "t6"
                       {:op :actuate-polymerization-reactor :effect :propose :subject "batch-001"}
                       coordinator))

    (println "== schedule-maintenance mnt-2 on compounder-002 (UNVERIFIED/unregistered compounding extruder -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on reactor-001 with :actuate-reactor? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated polymer-grade -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:polymer-grade :unobtainium}}
                       coordinator))
