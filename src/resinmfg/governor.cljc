(ns resinmfg.governor
  "Primary Forms Plant Operations Governor -- the independent
  compliance layer that earns the ResinAdvisor the right to commit.
  ...
  draft-schedule) the polymerization/compounding reactor, whether a
  ...
  coordination proposal and becomes direct reactor/polymerization-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:primary-forms-plant-operations-governor` (see
...
(def allowed-proposal-effects
  ...
  -- all four are propose-shaped drafts, NEVER a direct reactor/
  polymerization-line-equipment-control effect."
...
(defn- reactor-control-blocked-violations
  ...
  Anything else (direct polymerization-reactor/compounding-line
  control, a fabricated actuation effect) is this actor's central
  scope boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :reactor-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は重合反応器・コンパウンド設備等の直接操作に該当する可能性があり、恒久的に禁止")}]))

(defn- reactor-actuate-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:actuate-reactor? true` is attempting
  to directly actuate the polymerization/compounding reactor -- this
  actor may only ever propose/schedule a DRAFT maintenance window,
  never actuate the reactor directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-reactor? (:value proposal))))
    [{:rule :reactor-actuate-blocked
      :detail "重合/コンパウンド反応器の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- invalid-polymer-grade-violations
  ...
  let a fabricated polymer grade through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [polymer-grade (:polymer-grade (:value proposal))]
      (when (and (some? polymer-grade) (not (registry/polymer-grade-valid? polymer-grade)))
        [{:rule :invalid-polymer-grade
          :detail (str polymer-grade " は既知の polymer-grade 値ではない")}]))))
...
(defn check
  "Censors a ResinAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (reactor-control-blocked-violations proposal)
                           (reactor-actuate-blocked-violations request proposal)
