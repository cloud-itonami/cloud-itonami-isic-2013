(ns resinmfg.operation
  "ResinOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (ResinAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Primary Forms Plant
...
(defn build
  "Compiles a ResinOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `resinmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ResinAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Primary Forms Plant Operations Governor -- independent censor.
      (g/add-node :govern
