(ns resinmfg.store
  ...
  self-contained (no external primary-forms-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             polymer-grade/weight/off-spec-rate
    ...
    - `equipment`         -- a polymerization-reactor/compounding-line
    ...
    - `maintenance`       -- a scheduled polymerization/compounding-
                             reactor maintenance-window DRAFT against
    ...
    - `shipments`         -- a proposed outbound resin/synthetic-
                             rubber primary-form shipment DRAFT
                             (`resinmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (resinmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (resinmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :polymer-grade :pp :output-form :pellet
                :material "Polypropylene Homopolymer Pellets"
                :weight-kg 50000.0 :off-spec-rate-percent 1.2
                :verified? true :registered? true
                :shipped-weight-kg 10000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :polymer-grade :sbr :output-form :latex
                :material "Styrene-Butadiene Rubber Latex"
                :weight-kg 8000.0 :off-spec-rate-percent 0.6
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :polymer-grade :pe-hdpe :output-form :granule
                :material "HDPE Injection-Grade Granules"
                :weight-kg 60000.0 :off-spec-rate-percent 2.1
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"reactor-001" {:id "reactor-001" :kind :polymerization-reactor
                  :verified? true :registered? true
                  :last-maintenance-date "2026-05-01"}
   "compounder-002" {:id "compounder-002" :kind :compounding-extruder
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `resinmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `resinmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered polymerization-
  reactor unit (schedulable for maintenance), one UNVERIFIED/
  unregistered compounding-extruder unit (blocks any maintenance
