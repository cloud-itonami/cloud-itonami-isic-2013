(ns resinmfg.operation-test
  "Smoke tests for the compiled ResinOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `resinmfg.governor-contract-test`; the Store contract in
  `resinmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "ResinOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "reactor-001" :severity :moderate :description "monomer odor near polymerization reactor"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
