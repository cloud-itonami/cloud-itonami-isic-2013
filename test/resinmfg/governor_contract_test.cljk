(ns resinmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the polymerization/compounding
  reactor directly... does NOT authorize or execute maintenance/
  reactor operations') implemented faithfully. The single invariant
  under test:

    ResinAdvisor never schedules maintenance, flags a safety concern,
    or coordinates a shipment the Primary Forms Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:polymer-grade :pp}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :pp (:polymer-grade (store/batch db "batch-001"))) "SSoT actually updated")
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t2"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                     :value {:equipment-id "reactor-001" :maintenance-type :agitator-inspection
                             :scheduled-date "2026-08-01" :actuate-reactor? false}}
                    coordinator)]
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-polymerization-reactor :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t5"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-2"
                     :value {:equipment-id "compounder-002" :maintenance-type :die-inspection
                             :scheduled-date "2026-08-01" :actuate-reactor? false}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (not= :interrupted (:status res)))
...
(deftest tensile-test-out-of-tolerance-is-held
  (testing "ADR-2607999900: a batch whose own recorded :specimen-mass-kg yields a REAL physics-2d-simulated peak tensile load below the disclosed floor -> HOLD, wired into the SAME :coordinate-shipment proposal-gating function every other HARD check runs through"
    (let [[db actor] (fresh)
          _ (store/commit-record! db {:effect :batch/upsert :path ["batch-001"]
                                      :value {:specimen-mass-kg 0.05}})
...
(deftest tensile-test-in-tolerance-does-not-block-shipment
  (testing "a batch whose own recorded :specimen-mass-kg clears the real disclosed floor is unaffected by this check -- still escalates for human approval like any clean shipment coordination, proving this ADR is purely additive"
    (let [[db actor] (fresh)
          _ (store/commit-record! db {:effect :batch/upsert :path ["batch-001"]
                                      :value {:specimen-mass-kg 0.6}})
...
(deftest missing-specimen-mass-never-blocks-shipment
  (testing "a batch with no :specimen-mass-kg on file (no tensile-test specimen molded/tested yet) never trips the new check -- missing telemetry != violation, still escalates normally like any clean shipment"
    (let [[db actor] (fresh)
      ...
      (is (nil? (:specimen-mass-kg (store/batch db "batch-002")))
          "batch-002's seeded sample data never carried this field"))))

(deftest reactor-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-reactor? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
          res (exec-op actor "t8"
                    {:op :schedule-maintenance :effect :propose :subject "mnt-3"
                     :value {:equipment-id "reactor-001" :maintenance-type :force-run
                             :scheduled-date "2026-09-01" :actuate-reactor? true}}
                    coordinator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:reactor-actuate-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "reactor-001" :maintenance-type :agitator-inspection
                                          :scheduled-date "2026-08-01" :actuate-reactor? false}} coordinator)
          _ (approve! actor "t9a")
...
(deftest invalid-polymer-grade-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:polymer-grade :unobtainium}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-polymer-grade} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium (:polymer-grade (store/batch db "batch-001"))) "fabricated polymer-grade never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "reactor-001" :severity :moderate
                                            :description "monomer odor near polymerization reactor"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "reactor-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:polymer-grade :fabricated-grade}} coordinator)
